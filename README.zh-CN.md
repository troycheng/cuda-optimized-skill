...
Controller 还会绑定实际启动程序、项目内容、假设代际和已提交账本的摘要，防止恢复时悄悄换掉
执行入口或既有证据。
...
或缺少能力时会停止而不是猜测。项目内容、adapter 启动程序、假设代际和已提交账本头都带摘要
校验。方向实验只在项目副本中运行；这是协作式隔离，不是 OS 安全沙箱。

最终补丁把实践中暴露的时间浪费改成了运行时门禁：候选开始前必须先跑用户原始业务
baseline，并从当前最优实现继承所有已验证机制。适用于当前 claim 的阶段按成本递增：静态
检查、正确性、短版性能、必要的 profiler、正式测试；只有 serving claim 才进入服务测试。
前一步失败，后续阶段不会启动。同一机制改名或只调整参数不能重复占用候选额度。长任务会
输出心跳和明确的停止原因；多方 AI 审查只在本地证据已支持晋级时并行执行，总等待不超过
180 秒，并保留完整回答与失败记录。
