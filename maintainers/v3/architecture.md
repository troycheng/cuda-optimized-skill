...
- 精确架构和硬件特性，框架及版本范围；
- 可满足的完整信号组、反向信号、前置证据类别；
- 冲突方法、正文 UTF-8 字节成本、风险；
- Controller 必须执行的门禁、验证步骤、停止条件、证据状态；
- 来源、许可、最后复核日期。
...
能力库只保存稳定知识、决策线索和验证办法。版本敏感事实仍需从本机工具或一手文档确认。离线环境可使用带来源和复核日期的快照；过期或不匹配的条目降级为 `unverified`。历史重放以 `as_of` 为时间边界，能力卡或任一来源晚于该时点时标为 `unverified_future`，不能把未来知识写回旧评测。

检索只决定“是否值得加载这段知识”，不决定候选能否执行。能力查询不会输出
`candidate_admissible`；它返回命中的完整信号组、缺少的证据类别和分阶段的
`gate_requirements`。`pre_execution` 只控制能否开始实验，`promotion` 只在候选
执行后控制能否晋级，避免要求实验结果在执行前已经存在。阶段 3 由 Controller
使用绑定合同和哈希的真实产物解析这些门禁。能力查询用正文的精确 UTF-8 字节数
执行硬预算，并在安装自检中复算；模型 tokenizer 的实际消耗另由评测记录，不能由
作者低报，也不能把启发式 token 估算冒充硬上限。

门禁证据采用封闭的 adapter 专用格式。Controller 重新读取产物并校验哈希、合同、
环境、目标源码、候选源码、精确架构和记录时间；摘要只从追加账本中的封存事件派生，
