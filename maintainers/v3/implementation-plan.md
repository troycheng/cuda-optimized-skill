...
状态：实现与验证完成，待双端发布，2026-07-19

本计划以 `v2.9.0`（`8aa4a50`）为对照基线。基线测试套件共 813 项，其中本地 808 项通过、5 项因缺少物理 GPU 跳过。开发在 `codex/v3` 隔离 worktree 中进行，不向原始上游仓库推送。

## 完成摘要

- 阶段 0—4 已完成：合同、评测清单、长期控制器、追加账本、能力注册表、证据适配器、Planner 准入、稳定性校准、周期审计和重放恢复均有自动化测试。
- 阶段 5 已完成当前发布范围：RTX 5090 新鲜容器 lane 15/15 通过；真实同 kernel 配对校准因噪声过高停在 `CALIBRATING`，验证了 fail-closed，而不是制造性能结论。
- 阶段 6 已完成实现与文档：独立前向测试未发现阻塞问题，中英文 README、用户文档、运行协议和发布记录均已更新。
- 五臂真实 workload 效果对照仍是发布后的持续评测，不把当前控制逻辑与故障注入测试写成“3.0 一定优于 2.9”的证据。
...
已完成：能力查询结果可重放校验；观察摘要有数量和精确 UTF-8 字节
...
当前发布版接通 PyTorch 与 Nsys 诊断、编译、正确性、配对测量和 workload replay 的
封闭适配路径。NCU 无权限会精确降级为 `ERR_NVGPUCTRPERM`。独立账号、容器或密钥
服务属于高保证部署选项，不是默认本地 Controller 的安全承诺。
...
发布前必须重新运行全量本地测试、skill 自检、diff 检查和双远端 dry-run。随后只将
`codex/v3` 合并到用户 fork 与已授权内网镜像的主干，并同步本机安装的 skill。原始
上游保持 push disabled。
