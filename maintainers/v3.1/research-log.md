...
### RTX 5090 readiness 实测

2026-07-20 在物理 RTX 5090 / `sm_120` 上使用固定镜像
`sha256:a2d9d89bc4394eab3fadc62c6b5b3f739b6494c1f64c56f5ba5e6c008252a0e5`
完成新鲜 lane，18/18 通过，用时 52.141 秒。测试目录独立放在
`/data/tcheng/cuda-skill-e2e/v3.1`；没有修改驱动、counter 权限、频率、功耗或服务。GPU 1 在重试
前被 VLLM 占用，空闲检查拒绝启动，最终 lane 改用当时空闲的 GPU 7。

readiness 用时 8.793 秒，首次 baseline artifact 出现在 run 启动后 9.297 秒。CUDA 13.3 target
编译、GPU 执行、`sm_120` SASS、Compute Sanitizer 零错误和真实 Triton workload correctness/KPI
smoke 均通过。镜像没有 Nsys，记录为 diagnostic degradation；NCU 以
CUDA Driver API 的 `cuProfilerStart/cuProfilerStop`、`regex:readiness_kernel` 和单 launch 约束
运行真实 target range，返回 `ERR_NVGPUCTRPERM`，记录为
`user_action_required`。required 能力仍全部 ready，因此可以开始 workload diagnosis；没有尝试
sudo 或更改宿主机策略。

超时、workload smoke 失败、requirements 摘要错误三种故障注入都在 baseline 前阻断。摘要错误
留下 `requirements_digest_mismatch` 安装回执；修复次数和重复 probe 都为 0。

历史 V3.0 同 fixture 的首次 baseline 约为 0.014 秒，完整 lane 15/15、34.307 秒，因为它没有
readiness。V3.1 当前数据明确增加约 9 秒启动成本，不能据此声称“更快找到有效方向”。对照已保存
为远端 `final2/readiness-comparison.json`。速度收益必须在用户真实长 workload 上以工具返工、无效
profile 轮次和首次有效方向时间重新验证；这个 micro fixture 只证明准入顺序、证据质量和故障边界。

首轮 fixture 使用 runtime `cuda_profiler_api.h`。固定镜像的 `/opt/cuda-13.3` 编译前缀没有该
header；`/usr/local/cuda` 的旧 13.0 前缀虽有，但不能为 13.3 编译混用。NVIDIA CUDA 13.3
Driver API 文档仍保留 `cuProfilerStart/cuProfilerStop`，因此不能把工具包布局问题表述成 CUDA
...
2026-07-20 完成发布前复审。NVIDIA Nsight Systems 当前文档再次确认导出的 SQLite schema
可能随版本变化、表会按采集内容出现，且 `nsys analyze` 可用于采集后分析；因此 adapter 版本、
export schema 和实现摘要继续作为 epoch 身份的一部分，缺失表不能补零。Nsight Compute 13.3
文档说明 replay、缓存和 metric set 会显著影响采集成本；证据选择器因此保留分层成本与剩余
profile 次数，不把一次全量 NCU 当作默认动作。

向已登录 Kimi 提交了不含代码、路径、账号、日志和 workload 的实现摘要。采纳四项具体反例：
同一 run 的主动诊断写操作必须加互斥锁；上下文中的结果索引必须回查原始结果和 catalog 摘要；
observed outcome 的支持/反对关系必须由 Controller 固定，不能由下一轮模型重新解释；
`project_sandbox` 会夸大隔离能力，改名为 `project_copy` 并明确它不是 OS 安全边界。

未采纳没有基线依据的固定 workload 数量和固定命中率阈值，也未把项目副本描述为恶意代码
隔离。V3.1 的机制完成与方向效果分开记录：本地和目标机可以证明闭环、恢复和失败边界；是否
减少真实 profile 轮次，只能由用户提供的代表性长 workload 验证。
