...
隔离环境 identity 读取实际工具 path/realpath/version/hash、driver 与 GPU 标识、隔离 Python、
distribution 清单和 dist-info RECORD，以及 uid、容器、可见设备和 counter 权限摘要。库存中的
`available` 只表示命令存在，`usable` 保持未知；真实可用性仍由 capability probe 给出。标准
venv 的 `bin/python` leaf symlink 可以在父目录 no-follow、解析目标为普通可执行文件且执行前后
身份一致时使用；requirements、合同和证据文件仍不跟随 symlink。
...
隔离依赖修复会改变环境身份。安装成功后必须重新生成完整 identity；如果 identity 未变化，
说明库存没有真实刷新，gate 直接阻断。identity 变化后，v1 保守地废弃本轮已有结果并从
foundation 开头重跑，不能只给修复后的单项 probe 绑定旧摘要。gate 在任何 probe 前持久化
绝对开始时间，并在执行安装前先持久化已用修复次数；进程中断和 resume 不能重置墙上时间或
重复消费安装授权。

capability probe 必须彼此独立，不得依靠前一个 probe 留下的 CUDA context、IPC 句柄或后台
进程。v1 只有显式的 foundation/workload 阶段顺序，不用隐式状态依赖扩成通用 DAG；确有共享
前置状态的检查应合并为同一个原子 probe。
