...
- [ ] 先写失败测试：伪造 evidence ID、旧 epoch 引用、关系冲突、depends cycle、单 metric
  升级、遗漏 unmodeled、无反驳问题、重复 hypothesis。
- [ ] Controller 使用 evidence catalog 重放引用和 evidence kind，不信任模型摘要。
- [ ] 输出有限、规范排序的 active set 和稳定 digest。
- [ ] 提交：`feat(v3.1): admit competing diagnosis hypotheses`
